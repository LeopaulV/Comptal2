...
import { CategoryChargesPanel } from '../../Association/CategoryChargesPanel';
import { EntreprisePDFService } from '../../../services/EntreprisePDFService';
import { computeCategoryChargesData } from '../../../utils/categoryCharges';

interface PostesEntreprisePanelProps {
...
export const PostesEntreprisePanel: React.FC<PostesEntreprisePanelProps> = ({
  project,
  transactions,
  categories,
  onProjectChange,
  ...
  const handleExportPdf = useCallback(async () => {
    ...
    try {
      await EntreprisePDFService.generateChargesReportPDF(project, categoryChargesData);
    } catch (err) {
    ...
    <div className="invoicing-panel">
      <div className="invoicing-card">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12, marginBottom: 16 }}>
          <h2>{t('invoicing.charges.title')}</h2>
          {project && (
            <button
            ...
            >
              {t('invoicing.charges.exportChargesPdf', 'Exporter PDF des charges')}
            </button>
          )}
        </div>
        ...
        {project ? (
          <>
            <div className="charges-mode-tabs" style={{ marginBottom: 16, display: 'flex', gap: 8 }}>
              ...
              >
                {t('invoicing.charges.modeRecurrent', 'Projection de charges')}
              </button>
              <button
                type="button"
              ...
              >
                {t('invoicing.charges.modeCategories', 'Charges réelles')}
              </button>
            </div>
              ...
              <>
                <div style={{ marginBottom: 16 }}>
                  <ProjectionConfig
                    ...
                    titleKey="invoicing.charges.configLabel"
                  />
                </div>
