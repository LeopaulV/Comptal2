...
import { ProjectionDataByAccount, AccountProjectionConfig } from '../../types/ProjectManagement';
...
interface BalanceStackedProjectionChartProps {
...
const BalanceStackedProjectionChart: React.FC<BalanceStackedProjectionChartProps> = ({
  data,
  accountConfigs,
  accountColors,
  accountNames = {},
  ...
  const monthlyData = useMemo(() => {
    // Trier les comptes par priorité pour l'ordre d'affichage
    const sortedConfigs = [...accountConfigs].sort((a, b) => a.priority - b.priority);
    const accountCodes = sortedConfigs.map(config => config.accountCode);

    if (data.length === 0 || accountConfigs.length === 0) {
      return { months: [], accountData: {} as Record<string, number[]>, accountCodes };
    }

    // Créer une map pour stocker les données mensuelles
    const monthlyMap: Map<string, Record<string, number>> = new Map();
    ...
    for (const point of data) {
      ...
      // Prendre le dernier solde de chaque compte pour ce mois
      for (const accountCode of accountCodes) {
        if (point.accountBalances[accountCode] !== undefined) {
          monthData[accountCode] = point.accountBalances[accountCode];
        }
      }
    }
    ...
    // Créer les données par compte
    const accountData: Record<string, number[]> = {};
    for (const accountCode of accountCodes) {
      accountData[accountCode] = sortedMonths.map(month => {
        const monthData = monthlyMap.get(month);
        return monthData?.[accountCode] || 0;
      });
    }
  ...
  const datasets: MixedDataset[] = useMemo(() => {
    if (!monthlyData.accountCodes || monthlyData.accountCodes.length === 0) {
      return [];
    }

    return monthlyData.accountCodes.map((accountCode) => {
      const accountName = accountNames[accountCode] || accountCode;
      const accountDataArray = monthlyData.accountData[accountCode];
      
      return {
        label: accountName,
        data: accountDataArray || [],
        backgroundColor: accountColors[accountCode] || '#808080',
  ...
  }, [monthlyData, accountConfigs, accountColors, accountNames]);
  ...
  const totalSoldes = useMemo(() => {
    ...
    return monthlyData.months.map((_, monthIndex) =>
      monthlyData.accountCodes.reduce((total, accountCode) => {
        const accountData = monthlyData.accountData[accountCode];
        return total + (accountData?.[monthIndex] || 0);
      }, 0)
    );
  }, [monthlyData]);
  ...
  const trendData = useMemo(() => {
    if (totalSoldes.length === 0) {
      return { trendData: [], coefficient: 0 };
    }
  ...
  const trendDataset: MixedDataset = useMemo(() => ({
    label: t('chart.trendLine', { coefficient: trendData.coefficient.toFixed(2), defaultValue: `Tendance (${trendData.coefficient.toFixed(2)})` }),
  ...
  const initialLimits = useMemo(() => {
    ...
    return calculateYAxisLimits([...datasets, trendDataset]);
  ...
  }), [initialLimits, calculateYAxisLimits, isDarkMode, t]);

  if (monthlyData.months.length === 0 || datasets.length === 0) {
    return (
      <div className="flex items-center justify-center h-full text-gray-500 dark:text-gray-400">
        <p>{t('projectManagement.charts.noData', 'Aucune donnée disponible')}</p>
      </div>
    );
  }
...
export default BalanceStackedProjectionChart;
